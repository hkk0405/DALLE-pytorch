...
class Decoder(nn.Module):
	...
	def __attrs_post_init__(self) -> None:
		...
		make_conv  = partial(Conv2d, device=self.device, requires_grad=self.requires_grad, use_float16=self.use_mixed_precision)
		make_blk   = partial(DecoderBlock, n_layers=n_layers, device=self.device,
				requires_grad=self.requires_grad, use_float16=self.use_mixed_precision)
